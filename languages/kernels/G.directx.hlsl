...
[numthreads(64, 1, 1)] void FORCE(uint3 gid : SV_DispatchThreadID) {
  u32 i = gid.y * 1024u * 64u + gid.x;
  if (i >= P.holes) { return; }
  u32 h = i;
...
[numthreads(64, 1, 1)] void TAGPOOL(uint3 gid : SV_DispatchThreadID) {
  u32 i = gid.y * 1024u * 64u + gid.x;
  u32 c = i;
  if (i >= P.cells) { return; }
  f32 keeps = cel[4u * P.cells + c];
...
[numthreads(64, 1, 1)] void TAGCOPY(uint3 gid : SV_DispatchThreadID) {
  u32 i = gid.y * 1024u * 64u + gid.x;
  if (i >= P.cells * P.A) { return; }
