...
__device__ i32 hop(f32* st, f32* cel, const float4* dir, const Par P, u32 c, u32 a) {
...
__device__ i32 back(f32* st, f32* cel, const float4* dir, const Par P, u32 c, u32 a) {
  i32 x = i32_of_u(c % P.N);
  i32 y = i32_of_u(c / P.N);
  i32 tx = x - i32_of_f(dir[a].z);
  i32 ty = y - i32_of_f(dir[a].w);
...
__device__ f32 facing(f32* st, f32* cel, const float4* dir, const Par P, u32 a, u32 b) {
...
__device__ f32 crossing(f32* st, f32* cel, const float4* dir, const Par P, u32 a, u32 c, u32 other) {
...
__device__ f32 land(f32* st, f32* cel, const float4* dir, const Par P, u32 a, u32 c, f32 w, f32 rays, f32 space, f32 folds, f32 mixed) {
  f32 dr = w * rays / 4.0;
...
__device__ f32 standing_of(f32* st, f32* cel, const float4* dir, const Par P, u32 z, u32 a, u32 c) {
  f32 had = 0.0;
  f32 whole = 0.0;
...
extern "C" __global__ void SIGMA(f32* st, f32* cel, const float4* dir, const Par P) {
  u32 i = blockIdx.x * blockDim.x + threadIdx.x;
...
extern "C" __global__ void CREATE(f32* st, f32* cel, const float4* dir, const Par P) {
  u32 i = blockIdx.x * blockDim.x + threadIdx.x;
  u32 c = i;
  if (i >= P.cells) { return; }
  ...
  f32 DEG = P.DEG;
  f32 dSpace = 0.0;
  f32 fires0 = clampf((1.0 - powf(rho, DEG)), 0.0, 1.0) * 1.0;
  if (fires0 > 0.0) {
    for (u32 a = 0u; a < P.A; a++) {
...
extern "C" __global__ void MEET(f32* st, f32* cel, const float4* dir, const Par P) {
  u32 i = blockIdx.x * blockDim.x + threadIdx.x;
  u32 c = i;
  if (i >= P.cells) { return; }
...
extern "C" __global__ void POOLSUM(f32* st, f32* cel, const float4* dir, const Par P) {
  u32 i = blockIdx.x * blockDim.x + threadIdx.x;
  u32 c = i;
  if (i >= P.cells) { return; }
...
extern "C" __global__ void FORCE(f32* st, f32* cel, const float4* dir, const Par P) {
  u32 i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= P.holes) { return; }
  u32 h = i;
...
extern "C" __global__ void TAGPOOL(f32* st, f32* cel, const float4* dir, const Par P) {
  u32 i = blockIdx.x * blockDim.x + threadIdx.x;
  u32 c = i;
  if (i >= P.cells) { return; }
  f32 keeps = cel[4u * P.cells + c];
...
extern "C" __global__ void TAGCOPY(f32* st, f32* cel, const float4* dir, const Par P) {
  u32 i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= P.cells * P.A) { return; }
