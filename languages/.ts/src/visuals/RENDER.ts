...
const registry = async () => {
  ...
  /* THE THEORIES THIS PACKAGE HAS, which is one - the research repository carries the
     variants it compares G against and this one is G on its own. A theory that declares no
     visuals contributes none rather than failing. */
  const theories: [string, string, any][] = [
    ["G", "G", (await import("../theories/G/G.ts")).G],
  ];
