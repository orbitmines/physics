...
/**
 * THE TRANSPORT LAW, WHICH IS THIS MODEL'S OWN — and here it is the RULES' own.
 *
 * The carrier's drift falls with the density it passes through, so flux conservation
 * Φ = 4πr²nv goes quadratic in n and the profile turns over from 1/r² to 1/r. Same algebra as
 * MOND's simple interpolation, arrived at from transport rather than assumed.
 *
 * AND IT IS NO LONGER TYPED HERE. `boost` is what `spreading` solves in `Prove`, closed off
 * `G.ts`: the speed a carrier goes is `n/(n + a_0)` because a ray with nowhere to step spends
 * the tick making the room instead, and `a_0` is the space line's own waiting term rather than
 * a number read off a report. A curve that moves, moved because the rules did.
 */
const transport = boost;
