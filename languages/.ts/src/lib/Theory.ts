/**
 * THE THEORY BUILDER, AS THE PACKAGE SHIPS IT.
 *
 * THIS IS `implementations/.ts/src/lib/Theory.ts` WITH TWO THINGS TAKEN OUT, and both
 * were taken out for the same reason: they reach out of the package. `visual` named a
 * painter from `visuals/CANVAS.ts`, which is a `CanvasRenderingContext2D` and a renderer
 * that is not shipped here; `theorems` read the generated `theorems/` folder off disk
 * with `node:fs`, which is a folder of the RESEARCH REPOSITORY and not of an installed
 * package. Neither is reachable from `G`, so neither is part of running it.
 *
 * WHICH MAKES THIS A SECOND COPY THAT CAN DRIFT, and that is the cost being paid
 * knowingly. The research repository is where a rule is changed; anything changed there
 * has to be brought across, and the two files are otherwise line-for-line so that a diff
 * says exactly what has moved.
 */
...
export class Rule<Type extends RuleType = []> {
  constructor(
    ...
    public where?: string,
    /**
     * THE DECLARATION THIS RULE WAS BUILT FROM — its quantifier, its gates and its action,
...
> {
  ...
  /**
   * A RULE, WRITTEN IN THE SUB-LANGUAGE — `at.point.of(neutral).makes(...)`.
   *
   ...
   *
   * THE RAW FORM IS STILL HERE, and is a different method on purpose. `Theory` is
   * infrastructure and something will always need a rule that this vocabulary cannot say; but
   * a rule written that way is INVISIBLE to the continuous reading and is reported as such
   * rather than quietly leaving a term out of the equation.
   */
  rule = <Name extends string>(name: Name, declared: Declared): Theory<TRules & { [K in Name]: Rule }, TWorld, L, R, B, TLayers, TVisuals> =>
    this.copy<TRules & { [K in Name]: Rule }>(copy => {
      copy.rules = {
        ...copy.rules,
        [name]: new Rule(declared.quantifier.type, declared.exec, declared.where, declared),
      };
    })

  /**
   * A RULE AS A BARE FUNCTION — for what the sub-language cannot yet say.
   *
   * It runs exactly as a declared one does and is exactly as much a rule of the theory. What
   * it is not is READABLE: `continuum` lists it under `opaque` and the line it assembles is
   * short of whatever this rule would have contributed. That is the honest failure mode —
   * a term missing and said to be missing, rather than a term guessed at.
   */
  opaquely = <Name extends string, Type extends RuleType>(name: Name, type: Type, exec: (...refs: RefTypes<Type, Vocab<L, R, B>["Local"], Vocab<L, R, B>["Ray"], Vocab<L, R, B>["Boundary"], TWorld>) => void, where?: string): Theory<TRules & { [K in Name]: Rule }, TWorld, L, R, B, TLayers, TVisuals> =>
    this.copy<TRules & { [K in Name]: Rule }>(copy => {
      copy.rules = {
