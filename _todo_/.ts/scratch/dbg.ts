...
console.log("exits", JSON.stringify(g.exits), "steering=", w.steering);
const zp = g.exits.findIndex((v:number[]) => v[0]===0&&v[1]===0&&v[2]===1);
const zm = g.exits.findIndex((v:number[]) => v[0]===0&&v[1]===0&&v[2]===-1);
const xp = g.exits.findIndex((v:number[]) => v[0]===1&&v[1]===0&&v[2]===0);
...
for (let t=0;t<12;t++) {
  w.tick(); lay();
  let at:any, ray:any;
  for (const l of w.locals) { for (const ry of (l as any).rays) if (ry.active&&ry.charge){ray=ry;break;}
    if (ray){at=w.embedding.at(l as any);break;} }
  console.log(t, at? at.map((x:number)=>x-C):null, "turned=",ray?.turned, "gyro=",ray?.gyrophase?.toFixed?.(2),
    "steered=",w.steered, "turnsTaken=",w.turnsTaken);
  if(!at) break;
}
