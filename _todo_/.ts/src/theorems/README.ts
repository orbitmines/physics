...
And one more since: \`term\`, which says that a quantity is one term of an equation and names
the rewrite it came out of. It is there because \`vacuum.equation\` - the whole continuous model
on one line - was a DEFINITION, a string with every term already in it, true because somebody
transcribed it; and \`atom.emission\`, which writes a hydrogen state into the one term of that
line no rule puts there, was another. Stated a term at a time off the solver's own rule set,
with each term ablated to show it moves the vacuum at all, the line is what \`every rule is a
term, and the terms add\` ADDS UP TO, and "Sigma is the only term that is not a rule" is a count
over the terms rather than a sentence. Both theorems now derive from measured leaves and have
no definitions in them at all.
