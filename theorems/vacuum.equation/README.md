...
| theory | answer |
|---|---|
| `G` | (\partial_t + \hat{d}·\nabla_x + \paren{\nabla n_f}·\nabla_{\hat{d}})n = \nu\paren{1 - \rho}^(DEG) - \sigmaFn\tilde{n} + \sum_bm̄_b\delta\paren{x - x_b} |
...
**\sum_bm̄_b\delta\paren{x - x_b} = carried**  
...
space: 0
degree: 0
term: + \sum_bm̄_b\delta\paren{x - x_b}
```
...
**\hat{d}·\nabla_x + \paren{\nabla n_f}·\nabla_{\hat{d}} + \partial_tn = \sum_bm̄_b\delta\paren{x - x_b} + \nu\paren{1 - \rho}^(DEG) + \sigma\paren{1 - \omega}n - \sigmaFn\tilde{n}**  
<sub>theorem · and the terms add</sub>  

```
+ \sum_bm̄_b\delta\paren{x - x_b}   (not a rule)
