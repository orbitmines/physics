...
n_f = \paren{\text{the } n_f \text{ where } F·\sigma·\omega·\rho^(2) - DEG·\nu·\paren{1 - \rho}^(DEG)·\paren{1 - \paren{1 - \frac{1}{DEG}}^{n_f}} = 0} + \frac{\paren{1 - \beta}·m̄_x·A·\paren{1 - \paren{1 - \sigma·\omega·\rho}^(R̄)}·r^{-\paren{D - 2}}}{\sigma·\omega·\rho}, which grows with how far a ray has come
```
