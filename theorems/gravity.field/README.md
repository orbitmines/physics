...
> Einstein's equation relates the curvature to the source. Do these rules give that relation, and with what coupling?
...
| theory | answer |
|---|---|
| `G` | the field equation = 1 - \paren{1 - \Phi}^(2) |
...
**\delta n_f = \frac{l.choose\paren{m̄_x·l.DEG·\paren{1 - \beta}}·\omega·A·\paren{1 - \paren{1 - \sigma·F·\rho}^(R̄)}·r^{-\paren{D - 2}}}{\sigma·F·\rho}**  
...
```
the record a body adds is \delta n_f = \frac{l.choose\paren{m̄_x·l.DEG·\paren{1 - \beta}}·\omega·A·\paren{1 - \paren{1 - \sigma·F·\rho}^(R̄)}·r^{-\paren{D - 2}}}{\sigma·F·\rho}
...
```
A = \paren{1 - \Phi}^(2), so 1 - A = 1 - \paren{1 - \Phi}^(2)
Einstein: 1 - A = 2\Phi
so this model is Einstein's times \frac{\paren{1 - \paren{1 - \Phi}^(2)}}{2·\Phi}
```
