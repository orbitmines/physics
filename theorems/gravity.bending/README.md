...
**\alpha = \frac{2·\paren{1 - \beta}·m̄_x·A·\paren{1 - \paren{1 - \sigma·F·\rho}^(R̄)}·b^{-\paren{D - 2}}}{\sigma·F·\rho}**  
<sub>theorem · the integral of the index across the path</sub>  

```
turns: 1 way straight on against folds[d] ways out along d
the fold record along the path: \frac{\paren{1 - \beta}·m̄_x·A·\paren{1 - \paren{1 - \sigma·F·\rho}^(R̄)}·r^{-\paren{D - 2}}}{\sigma·F·\rho}
